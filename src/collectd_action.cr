...
class CollectDAction
  ...
  SUPPORTED_TAGS = { "cpu-0" => ["cpu-idle"], "load" => ["load"], "memory" => ["memory-used", "memory-buffered", "memory-cached","memory-free"], "df-root" => ["percent_bytes-free","percent_bytes-reserved"] }
  ...
  private def handlle_output(data_struct : CollectDData)
    ...
    first_tag = tag[0]
    second_tag = tag[1]
    if first_tag && SUPPORTED_TAGS[first_tag]
